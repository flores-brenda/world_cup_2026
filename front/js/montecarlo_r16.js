// ══════════════════════════════════════════════════════════
// MONTE CARLO OCTAVOS — Simulation starting from Round of 16
// ══════════════════════════════════════════════════════════

const MC_R16_SIMULATIONS = 10000;
const MC_R16_BATCH_SIZE = 500;

// Helper: check real winner for a match
function getRealPlayedMatchWinnerMC(t1, t2) {
...
function mcSimulateKOMatchR16(teamA, teamB, ratings, globalAvg) {
...
// Correr un torneo completo desde el R16 (Octavos)
function mcRunOneTournamentR16(ratings, globalAvg) {
  // 1. Armar las parejas de R16
  let currentRound = [
    { t1: "Canada", t2: "Morocco" },
    { t1: "Paraguay", t2: "France" },
    { t1: "Brazil", t2: "Norway" },
    { t1: "Mexico", t2: "England" },
    { t1: "Portugal", t2: "Spain" },
    { t1: "United States", t2: "Belgium" },
    { t1: "Argentina", t2: "Egypt" },
    { t1: "Switzerland", t2: "Colombia" }
  ];
  ...
  for (const round of rounds) {
    ...
    for (const match of currentRound) {
      ...
      // Chequear si el match ya se jugó en la vida real
      const realW = getRealPlayedMatchWinnerMC(match.t1, match.t2);
      const winner = realW ? realW : mcSimulateKOMatchR16(match.t1, match.t2, ratings, globalAvg);
...
function runMonteCarloR16(onProgress, onComplete) {
  ...
  function runBatch() {
    const end = Math.min(done + MC_R16_BATCH_SIZE, MC_R16_SIMULATIONS);
    for (let s = done; s < end; s++) {
      const { champion, finalist1, finalist2, sfTeams, qfTeams } = mcRunOneTournamentR16(ratings, globalAvg);
    ...
    onProgress(done / MC_R16_SIMULATIONS);

    if (done < MC_R16_SIMULATIONS) {
...
function renderMonteCarloR16() {
  const container = document.getElementById('mc-container-r16');
  if (!container) return;
  ...
  container.innerHTML = `
    <div style="text-align:center; padding: 40px 20px;">
      ...
      <div style="font-family:'Bebas Neue',sans-serif; font-size:28px; color:var(--gold); letter-spacing:3px; margin-bottom:12px;">
        ${t('mc_intro_title')} (OCTAVOS)
      </div>
      <div style="color:var(--muted); font-size:14px; max-width:500px; margin:0 auto 28px; line-height:1.7;">
        ${t('sec_mc_r16_sub')}
      </div>
      <button id="btn-run-mc-r16" class="btn-primary" style="font-size:16px; padding:14px 40px; letter-spacing:2px;">
        ${t('mc_btn_run')}
      </button>
    </div>`;

  document.getElementById('btn-run-mc-r16')?.addEventListener('click', startMCSimulationR16);
}

function startMCSimulationR16() {
  const container = document.getElementById('mc-container-r16');
  if (!container) return;
  ...
  container.innerHTML = `
    <div style="text-align:center; padding:60px 20px;">
      ...
      <div style="background:var(--surface2); border-radius:8px; height:12px; max-width:400px; margin:0 auto 12px; overflow:hidden; border:1px solid var(--border);">
        <div id="mc-progress-bar-r16" style="height:100%; width:0%; background:linear-gradient(90deg,var(--blue),var(--gold)); border-radius:8px; transition:width 0.2s;"></div>
      </div>
      <div id="mc-progress-text-r16" style="color:var(--muted); font-size:13px;">0 / ${MC_R16_SIMULATIONS.toLocaleString()} ${t('mc_simulations')}</div>
    </div>
    <style>
  ...
  runMonteCarloR16(
    (pct) => {
      const bar  = document.getElementById('mc-progress-bar-r16');
      const txt  = document.getElementById('mc-progress-text-r16');
      if (bar) bar.style.width = (pct * 100).toFixed(1) + '%';
      if (txt) txt.textContent = `${Math.round(pct * MC_R16_SIMULATIONS).toLocaleString()} / ${MC_R16_SIMULATIONS.toLocaleString()} ${t('mc_simulations')}`;
    },
    (counts) => renderMCResultsR16(counts)
  );
}

function renderMCResultsR16(counts) {
  const container = document.getElementById('mc-container-r16');
  if (!container) return;

  const N = MC_R16_SIMULATIONS;
  ...
  top16.forEach((t_team, i) => {
    ...
    tableHtml += `
      <tr style="${rowBg}">
        ...
        <td style="text-align:center; color:var(--muted);">${t_team.qf.toFixed(1)}%</td>
        <td>
  ...
  container.innerHTML = `
    <div style="text-align:center; margin-bottom:24px;">
      <div style="font-size:11px; color:var(--muted); text-transform:uppercase; letter-spacing:2px; margin-bottom:6px;">
        ${t('mc_based_on')} <strong style="color:var(--gold);">${N.toLocaleString()} ${t('mc_simulations')}</strong> — ${t('mc_simulations_label')} (Desde Octavos)
      </div>
    ...
    <div style="text-align:center; margin-top:16px;">
      <button id="btn-rerun-mc-r16" class="btn-primary" style="background:var(--surface2); border:1px solid var(--border); color:var(--muted); font-size:13px; padding:10px 28px;">
        ${t('mc_btn_rerun')}
      </button>
    </div>`;

  document.getElementById('btn-rerun-mc-r16')?.addEventListener('click', startMCSimulationR16);
}

function initMonteCarloR16Module() {
  renderMonteCarloR16();
}
